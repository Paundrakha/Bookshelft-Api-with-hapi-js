...
const getAllBooks = (request, h) => {
  ...
  if (name) {
    const filterNamaBuku = books.filter((book) => {
  ...
  if (reading) {
    const filterBacaBuku = books.filter((book) => Number(book.reading) === Number(reading),
    );
    const response = h.response({
      status: 'success',
      data: {
        books: filterBacaBuku.map((book) => ({
          id: book.id,
          name: book.name,
          publisher: book.publisher,
        })),
